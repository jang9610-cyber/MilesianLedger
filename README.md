...
개발 빌드의 PIP **영역 촬영**은 마우스로 지정한 영역에서 여러 아이템 이름을 읽어 개당 최저가를 표시합니다. 촬영 결과에는 저장된 경매장 자료에서 품목이나 후보를 찾은 이름만 표시합니다. 후보가 없는 화면 안내·알 수 없는 문구는 결과와 이름 수정창에서 제외하고, 현재 매물이 없는 알려진 품목은 유지합니다. 애매한 이름은 후보를 선택하기 전까지 가격을 확정하지 않습니다. Windows의 한국어 OCR로 이 PC에서 처리하며, 촬영 이미지와 인식 문장은 파일로 저장하거나 서버에 전송하지 않습니다. 사용 순서와 요구 사항은 [PIP 사용 안내](docs/USER_GUIDE.md#영역-촬영으로-여러-아이템-찾기-개발-빌드)를 참고하세요.

PIP에서 **전체 촬영 단축키 사용**을 켜면 **Alt+Home**으로 마우스가 있는 모니터 전체를 촬영합니다. 기본은 꺼짐이며 Alt+End·Alt+Insert로 바꿀 수 있습니다. Windows Raw Input으로 백그라운드 입력을 받아 포커스 전환·영역 선택 없이 촬영을 시도하며, 실제 마비노기 실행 중 입력 수신 여부는 게임 환경에서 확인해야 합니다. 단축키 패널의 **입력 수신 · n회**로 수신 여부를 확인하고 **다시 연결**로 재시도할 수 있습니다. 전역 키보드 후킹·게임 입력 생성·키 입력 기록 저장은 하지 않습니다.
