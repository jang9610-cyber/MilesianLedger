...
$taskOutput = Join-Path $taskRoot ('artifacts/market-search-verification/' + [guid]::NewGuid().ToString('N'))
New-Item -ItemType Directory -Path $taskOutput -Force | Out-Null
$taskRunner = Join-Path $taskOutput 'MarketSearchVerificationRunner.exe'
...
if ($LASTEXITCODE -ne 0) { throw 'Market search verification compilation failed.' }
& $taskRunner $taskOutput
if ($LASTEXITCODE -ne 0) { throw 'Market search verification failed.' }
