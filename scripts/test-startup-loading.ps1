...
$taskOutput = Join-Path $taskRepo ('artifacts/startup-loading-ui/' + [Guid]::NewGuid().ToString('N'))
...
$taskRunner = Join-Path $taskRuntime 'StartupLoadingVerificationRunner.exe'
...
$taskArgs += Join-Path $taskRepo 'tests/StartupLoadingVerificationRunner.cs'
& (Join-Path $taskFramework 'csc.exe') @taskArgs
if ($LASTEXITCODE -ne 0) { throw 'Startup loading UI verification compilation failed.' }
...
if (-not $taskProcess.WaitForExit(45000)) { $taskProcess.Kill(); throw 'Startup loading UI verification timed out.' }
...
if ($taskProcess.ExitCode -ne 0) {
    ...
    throw ('Startup loading UI verification failed. Artifacts: ' + $taskOutput)
}
Write-Output ('Startup loading UI artifacts: ' + $taskOutput)
